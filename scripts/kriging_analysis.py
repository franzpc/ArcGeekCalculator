...
class KrigingAnalysisAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterNumber(
                ...
                self.tr('Output cell size (use larger values for projected CRS in meters, e.g. 100-1000)'),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        # PyKrige builds a distance matrix of shape (n_grid_cells, n_data_points).
        # Each element is a float64 (8 bytes), so memory grows very fast.
        # Limit to 2,000,000 cells to avoid out-of-memory crashes on typical hardware.
        MAX_GRID_CELLS = 2_000_000
        total_cells = grid_width * grid_height
        if total_cells > MAX_GRID_CELLS:
            # Suggest the minimum cell size that would keep the grid within the limit
            min_safe_cell_size = math.sqrt(
                (extent.width() * extent.height()) / MAX_GRID_CELLS
            )
            raise QgsProcessingException(
                self.tr(
                    f'The output grid is too large ({grid_width} x {grid_height} = {total_cells:,} cells). '
                    f'This would require several gigabytes of RAM and will crash QGIS.\n\n'
                    f'Please increase the cell size. '
                    f'For your current extent, the minimum recommended cell size is '
                    f'{min_safe_cell_size:.1f} (in the same units as your CRS).\n\n'
                    f'Tip: For projected CRS (meters), use values like 500, 1000 or larger '
                    f'depending on your study area size.'
                )
            )
        # --- END GRID SIZE SAFETY CHECK ---
